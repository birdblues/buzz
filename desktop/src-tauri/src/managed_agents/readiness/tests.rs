//! Readiness tests. Split out of `readiness.rs` so that module stays
//! under the desktop file-size ratchet; the same reason the OpenRouter
//! cases already live in `tests/openrouter_tests.rs`.
