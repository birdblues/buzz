...
/// A 1x1 PNG with no ancillary chunks, so the upload path's scrub leaves it
/// exactly as it is and a test can still assert on the bytes it sent.
  ...
  group('ComposeBar', () {
    ...
    for (final statusCode in [
      ...
      testWidgets('shows friendly copy for a $statusCode upload response', (
        ...
        expect(
          find.text('The server rejected this image. Try exporting it again.'),
          findsOneWidget,
        );
