...
macos_overrides="$repo_root/mobile/macos/Flutter/WorktreeOverrides.xcconfig"
...
if [[ "$in_worktree" != "1" ]]; then
    rm -f "$ios_overrides" "$macos_overrides" "$android_props"
    exit 0
fi
...
macos_bundle_id="xyz.block.buzz.dogfood.client.${ios_slug}"
...
# The macOS client (fork-only target) follows the same convention; the
# directory exists only once `flutter create --platforms=macos` has run.
if [[ -d "$(dirname "$macos_overrides")" ]]; then
cat > "$macos_overrides" <<XCCONFIG
// Generated by scripts/mobile-worktree-overrides.sh — gitignored, do not edit.
// Applies to Debug builds only (included from Flutter/Flutter-Debug.xcconfig
// before AppOverrides.xcconfig, so a developer's app-specific overrides win).
BUNDLE_IDENTIFIER = ${macos_bundle_id}
APP_DISPLAY_NAME = Buzz Client (${label})
XCCONFIG
fi
...
echo "📱 Worktree ${worktree_name}: iOS label \"${label}\" (${ios_bundle_id}); macOS client \"Buzz Client (${label})\" (${macos_bundle_id}); Android label \"${android_app_name}\" (xyz.block.buzz.mobile${android_suffix})"
